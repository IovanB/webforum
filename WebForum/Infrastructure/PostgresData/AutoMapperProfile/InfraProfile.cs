    ...
    public class InfraProfile: Profile
    {
        public InfraProfile()
        {
           ...
           .ConstructUsing(src => new Domain.Entities.Topic(src.Id, src.Name, src.CategoryId));
            ...
            CreateMap<Domain.Entities.Post,Entity.Entities.Post >().ForMember(dest => dest.Id, opt => opt.Ignore())
            .ForMember(dest => dest.CreatedAt, opt => opt.MapFrom(src => DateTime.UtcNow))
            .ForMember(dest => dest.UpdatedAt, opt => opt.MapFrom(src => DateTime.UtcNow));
           ...
           .ConstructUsing(src => new Domain.Entities.Post(src.Id,src.Title, src.Content, src.TopicId));
            ...
            CreateMap<Domain.Entities.Comment, Entity.Entities.Comment>().ForMember(dest => dest.Id, opt => opt.Ignore())
            .ForMember(dest => dest.CreatedAt, opt => opt.MapFrom(src => DateTime.UtcNow))
            .ForMember(dest => dest.UpdatedAt, opt => opt.MapFrom(src => DateTime.UtcNow));
